...
export async function createProduct(data) {
  ...
  const result = await db.prepare(
    `INSERT INTO products (title, slug, description, content, price, compare_price, category, brand, sku, image_url, gallery, compatible_models, features, affiliate_link, meta_title, meta_description, is_featured, is_active, sort_order)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    data.title, data.slug, data.description || '', data.content || '',
    data.price || 0, data.compare_price || null,
    data.category || 'apparel', data.brand || '',
...
export async function updateProduct(id, data) {
  ...
  const allowedFields = ['title', 'slug', 'description', 'content', 'price', 'compare_price', 'category', 'brand', 'sku', 'image_url', 'affiliate_link', 'meta_title', 'meta_description', 'is_featured', 'is_active', 'sort_order'];
...
// ============================================
// Categories (Dynamic Product Categories & Sub-categories)
// ============================================
...
export async function getCategories({ includeInactive = false } = {}) {
  try {
    ...
    const where = includeInactive ? '' : 'WHERE is_active = 1';
    const result = await db.prepare(`SELECT * FROM categories ${where} ORDER BY sort_order ASC, name ASC`).all();
    return result?.results || [];
  } catch (err) {
...
export async function createCategory(data) {
  ...
  return await db.prepare(
    `INSERT INTO categories (name, slug, parent_id, description, image_url, icon, sort_order, is_active)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    data.name,
...
export async function updateCategory(id, data) {
  ...
  const fields = [];
  const values = [];
  const allowed = ['name', 'slug', 'parent_id', 'description', 'image_url', 'icon', 'sort_order', 'is_active'];
...
export async function deleteCategory(id) {
  const db = getDB();
  if (!db) throw new Error('Database not available');
  return await db.prepare('DELETE FROM categories WHERE id = ?').bind(id).run();
}
...
export async function getPages({ published = true } = {}) {
  try {
    ...
    console.error('getPages DB error:', err);
    return [];
  }
}
...
export async function getPageBySlug(slug) {
  try {
    ...
    console.error('getPageBySlug DB error:', err);
    return null;
  }
}
...
export async function getPageById(id) {
  try {
    ...
    console.error('getPageById DB error:', err);
    return null;
  }
}
...
export async function getRedirects({ activeOnly = false } = {}) {
  try {
    ...
    console.error('getRedirects DB error:', err);
    return [];
  }
}
...
export async function getRedirectBySource(sourcePath) {
  try {
    ...
    console.error('getRedirectBySource DB error:', err);
    return null;
  }
}
...
export async function getSetting(key) {
  try {
    ...
    console.error('getSetting DB error:', err);
    return null;
  }
}
...
export async function getStats() {
  try {
    ...
    const [products, pages, redirects] = await Promise.all([
      db.prepare('SELECT COUNT(*) as count FROM products').first().catch(() => ({ count: 0 })),
      db.prepare('SELECT COUNT(*) as count FROM pages WHERE is_published = 1').first().catch(() => ({ count: 0 })),
      db.prepare('SELECT COUNT(*) as count FROM redirects WHERE is_active = 1').first().catch(() => ({ count: 0 })),
    ]);
    ...
    return { products: 0, pages: 0, redirects: 0 };
  }
}
